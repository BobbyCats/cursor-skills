...
| **Neo-Brutalist Editorial** | **新野兽主义、工业编辑** | **投资路演、硬核数据展示** |
...
4. **全屏快捷键（强制）** — 必须在键盘事件中加入 `F` 键切换全屏的逻辑，且**不要**在 UI 中添加任何全屏提示文字：
   ```javascript
   ...
   ```
5. **可选增强**（按风格）：自定义光标、粒子背景、视差效果、3D 倾斜、计数器动画
...
- 浏览器全屏：F11（Windows/Linux）或 Ctrl+Cmd+F（macOS），或直接按 `F` 键
...
| 元素重叠 | 不要使用绝对的百分比或固定像素，使用 `clamp()` 和 `grid-template-columns` 的自适应分配（如 `minmax()` 或 `clamp()`）防止文字和图表重叠。表格数据单元格（`td`）应加上 `white-space: nowrap` 防止换行。 |
